...
void dfs(int curr, int parent) {
    //initially set everything with 0 and max value
    for(int i =0;i<M;i++){
        par[curr][i]={0,INT_MAX};
    }
    par[curr][0].first = parent;
    par[curr][0].second = min(arr[curr],arr[parent]);
    depth[curr] = 1+ depth[parent];
    for(int j =1;j<M;j++){
        par[curr][j] = {par[par[curr][j-1].first][j-1].first,min(par[curr][j-1].second,par[par[curr][j-1].first][j-1].second)};
    }
...
int minDistance(int u, int v){
    ...
    //u we need to move to the same position as v
    ...
    for(int j = M-1;j>=0;j--){
        if(par[u][j].first!=par[v][j].first){
            mn = min({mn,par[u][j].second,par[v][j].second});
            u = par[u][j].first;
            v = par[v][j].first;
        }
    }
