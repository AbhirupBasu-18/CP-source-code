...
void dfs(int node, int parent,int& time,vector<vector<int>>& tree,vi& in, vi& out){
    in[node]=time++;
    for(auto it:tree[node]){
        if(it!=parent){
            dfs(it,node,time,tree,in,out);
        }
    }
    out[node]=time++;
}
void solve(int tc,int curr) {
    ...
    int time = 0;
    dfs(1,-1,time,tree,in,out);
    for(int i=1;i<=n;i++){
        euler[in[i]]=euler[out[i]]=val[i-1];
    }
    SegTree<Node1,Update1> seg = SegTree<Node1,Update1>(2*n,euler);
    lfr(i,0,q){
        int type;
        cin>>type;
        if(type==1){
            ...
            int inTime = in[k];
            int outTime = out[k];
            seg.make_update(inTime,u);
            seg.make_update(outTime,u);
        }
        else{
            int x;
            cin>>x;
            cout<<(seg.make_query(in[x],out[x]).val)/2<<endl;
        }
    }
}
